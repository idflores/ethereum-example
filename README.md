...
The *data* property in Ethereum *blocks*, like Bitcoin, is an accumulator of [*transactions*](#transactions-and-messages) which come in two types. Most notably, one type of transaction also has a data property which holds contract opcode to be executed by the [Ethereum Virtual Machine](#the-ethereum-virtual-machine). The lifecycle of an Ethereum transaction is further illustrated:
...
At its core, Ethereum is a "State Transition Machine" that uses *transactions* to change the current state of the blockchain or contracts on the blockchain. A **Transaction** is a signed data structure from an *External Account*. The origination is what gives the *transaction* its definition; a transaction may occur from an External Account to another External Account or to a contract but never from a contract. A transaction has the following architecture:
+ Recipient Address
