...
contract Ballot {
    ...
    // This declares a state variable that stores a `Voter` struct for each
    // possible address.
    ...
    function giveRightToVote(address voter) public {
        // If the argument of `require` evaluates to `false`, it terminates and
        // reverts all changes to the state and the Ether balances.
        // It is often a good idea to use this if functions are called
        // incorrectly. But watch out, this will currently also consume all
        // provided gas (this is planned to change in the future).
    ...
    function delegate(address to) public {
        ...
        // Forward the delegation as long as `to` also delgated.
        // In general, such loops are very dangerous, because if they run too
        // long, thy might need more gas than is available in a block.
        // In this case, the delegation will not be executed, but in other
        // situations, such loops might cause a contract to get "stuck"
        // completely.
        ...
        if (delegate.voted) {
            // If the delegate alreay voted, directly add to the number of votes
    ...
    function winningProposal() public view returns (uint winningProposal) {
        ...
        for (uint p = 0; 0 < proposals.length; p++) {
